...
const io = new socketio.Server({ maxHttpBufferSize: config.MAX_IMAGE_SIZE });
...
(async () => {
    ...
    io.attach(https.createServer(httpsOptions, app).listen(config.HTTPS_PORT, () => console.log(`Listening on ${sutils.IPV4Address()}:${config.HTTPS_PORT} (HTTPS)`)));
